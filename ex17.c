...
/* On a 64bit Linux machine with 1 row the size of the database is
 * 1032 bytes 
 * This equals to 512*2 + 4 + 4 
 * This represents the size of one address row!
 */
struct Address {
    int id; //size 4
    int set; // size 4
    char name[MAX_DATA]; //size 512
    char email[MAX_DATA]; // size 512
};
...
int main(int argc, char *argv[])
{
    ...
    switch(action) {
        case 'c':
            if (argc != 5) 
                die("Required: MAX_ROWS MAX_DATA", NULL);
